	  ...
	  const result = await webRForLeaflet.evalR(`
		if (exists("leaflet_map")) {
		  ...
		  html_content <- paste0(
			...
			'\\n',
			'                    // Verbesserte Popup-Extraktion - prüfe verschiedene Strukturen\\n',
			'                    if (call.args.length > 2) {\\n',
			'                      // Durchsuche alle Argumente nach Popup-Daten\\n',
			'                      for (var i = 2; i < call.args.length; i++) {\\n',
			'                        var arg = call.args[i];\\n',
			...
			'                        }\\n',
			'                        \\n',
			'                        // Objekt mit Popup-Eigenschaft\\n',
			'                        if (typeof arg === "object" && arg !== null) {\\n',
			'                          // Prüfe verschiedene mögliche Popup-Schlüssel\\n',
			...
			'                          if (popup) break;\\n',
			'                          \\n',
			'                          // Fallback: prüfe alle Objekt-Eigenschaften\\n',
			'                          var keys = Object.keys(arg);\\n',
			'                          for (var k = 0; k < keys.length; k++) {\\n',
			'                            var key = keys[k];\\n',
			'                            var value = arg[key];\\n',
			'                            \\n',
			'                            // Prüfe ob es ein Array oder String ist, der Popup-Daten enthalten könnte\\n',
			...
			'                          var marker = L.marker([lat[i], lng[i]]).addTo(map);\\n',
			'                          \\n',
			'                          // Popup hinzufügen falls vorhanden\\n',
			'                          if (popup) {\\n',
			'                            var popupContent;\\n',
			...
			'        }\\n',
			'\\n',
			'        // Füge Standard-Tiles hinzu falls keine addTiles-Aufrufe vorhanden\\n',
			'        if (!tilesAdded) {\\n',
			'          var fallbackUrl = "https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}";\\n',
			'          var fallbackOptions = {\\n',
			'            attribution: "© Esri",\\n',
